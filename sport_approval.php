...
<body>
    <nav>
        ...
        <div class="texts-container">
            ...
            <p><u>Sport</u> Clearance approval process</p>
        </div>
        ...
        <div class="left-side-container">
          <div class="approval-details-container">
            ...
            <?php
              $msg="";
              $status_msg="";
                $sql="SELECT * FROM sport_record WHERE student_id='$manage_id'";
            ...
            <div class="detail allApprove">
                ...
                <?php
                  ...
                  $reg_query=mysqli_query($conn,"SELECT * FROM sport_final_approve_reject WHERE student_id='$manage_id' ") or die(mysqli_error($conn));
                  $rows=mysqli_num_rows($reg_query);
                  if($rows>0){
                    echo "<p style=color:red;>Already approved...</p>"; 
                  }else{
                    $date = date('Y-m-d H:i:s');
                  $sql = mysqli_query($conn,"INSERT INTO sport_final_approve_reject (student_id,status,signature,staff_name,date) VALUES('$manage_id','$status_msg','$file_name','".$_SESSION['sport_full_name']."','$date')") or die(mysqli_error($conn));
                    $sql_delete_request=mysqli_query($conn,"DELETE FROM request_sport WHERE id='$manage_id'") or die(mysqli_error($conn));
                    $sql_delete_request=mysqli_query($conn,"DELETE FROM sport_rejected_requests WHERE student_id='$manage_id'") or die(mysqli_error($conn));
                  ...
                  $status_msg="Rejected";
                  $sql = mysqli_query($conn,"INSERT INTO sport_rejected_requests (student_id,status,staff_username) VALUES('$manage_id','$status_msg','".$_SESSION['sport_full_name']."')") or die(mysqli_error($conn));
