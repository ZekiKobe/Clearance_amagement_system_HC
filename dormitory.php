...
session_start();
 ...
 if(!isset($_SESSION['dormitory_logged_in'])){
...
<html lang="en" dir="ltr">
  ...
  <head>
    <meta charset="utf-8">
    <title></title>
    <link rel="stylesheet" href="css/dormitory/dormitory_page.css">
    <link rel="stylesheet" href="css/dormitory/dormitorySmallScreen.css">
...
.table-of-record-container table button:hover{
  ...
  <body>
                   ...
                   <li>
                    <span class="icon"> <img src="icons/manage(copy).png" width="20px" height="20px" alt=""> </span>
                     ...
                     <div class="dropdown-container" id="dropdown-cont" style="flex-direction:column;">
                        <a href="#material-register">Record dorm property</a>
      ...
      <div class="main-home">
        <div class="top-container">
            ...
            <div class="top-title" style="display: flex; flex-direction: column; align-items:center; justify-content:center;">
                ...
                <p>students online clearance system-Dormitory</p>

            </div>
            <div class="profile-cont">
                <img src="<?php echo $_SESSION['dormitory_image']; ?>" alt="" width="60px" height="60px">
                <div class="dropdown">
                  <button onclick="myFunction()" class="dropbtn"><?php echo $_SESSION['dormitory_u_name'];?>&#9660;</button>
          ...
          <a name="dashboard">
           <div class="dashboard-container">
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT property_id FROM dorm_record ORDER BY student_id";
              ...
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT id FROM request_dormitory ORDER BY id";
                  $res=mysqli_query($conn,$stmt) or die(mysqli_error($conn));
                  $row=mysqli_num_rows($res);
                  ?>
                  <p><?php echo $row ?></p>
              ...
              <div class="dashboard">
                ...
                <?php
                $stmt="SELECT student_id FROM dormitory_final_approve_reject ORDER BY student_id";
                  $res=mysqli_query($conn,$stmt) or die(mysqli_error($conn));
                  $row=mysqli_num_rows($res);
                  ?>
                  <p> <?php echo $row; ?> </p>
              ...
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT student_id FROM dormitory_rejected_requests ORDER BY student_id";
                  $res=mysqli_query($conn,$stmt) or die(mysqli_error($conn));
                  $row=mysqli_num_rows($res);
                  ?>
                  <p> <?php echo $row; ?> </p>
           ...
           <div class="view-request-container">
             ...
             <?php
             $sql="SELECT * FROM request_dormitory INNER JOIN student ON request_dormitory.id = student.id";
             ...
             <div class="requests">
               ...
               <button>
                 <a href="dormitory_approval.php?manageid=<?php echo $student_id; ?>&fullName=<?php echo $fullName; ?>&reason=<?php echo $reason; ?>">Manage</a>
               </button>
            </div>
            <hr>
            <?php
           ...
           <div class="material-lend-register">
               <div class="top-title">
                 <h1 style="margin:10px;">Record dorm property</h1>
               </div>
               ...
               <div class="form-row">
                 ...
                 <div class="form-row">
                   ...
                   <input type="text" name="given_by" value="<?php echo $_SESSION['dormitory_full_name']; ?>" required>
                 </div>
                 ...
                 <?php
                     if(isset($_POST["register"]))
                     {
                       ...
                       $given_by=$_POST["given_by"];
                               $sql2=mysqli_query($conn,"INSERT INTO dorm_record(property_id,property_name, date_given, taken_by,student_id,given_by)
                               VALUES ('$pr_id','$p_name','$date_taken','$taken_by','$student_id','$given_by')") or die(mysqli_error($conn));
                               if($sql2)
                               {
                        ...
                        <div class="edit-delete-material-lend-container">
                         <div class="top-h1-add-container">
                           <h3 style="margin:10px;">Lists of property lend </h3>
                            ...
                            $query=mysqli_query($conn,"SELECT * FROM dorm_record WHERE CONCAT(property_name,student_id,taken_by) LIKE '%$filterValues%'") or die(mysqli_error($conn));
                            ...
                            if($rows_srch>0){
                              while ($rows_srch=mysqli_fetch_assoc($query)){
                                ...
                                $date_tkn=$rows_srch["date_given"];
                                $tkn_by=$rows_srch["taken_by"];
                                $stdnt_id=$rows_srch["student_id"];
                                $staff_name=$rows_srch["given_by"];
                                $table_name="dorm_record";
                                $header="dormitory.php#edit-delete-lend";
                                ?>
                                <tr>
                       ...
                       <button style="background-color: red; color:white;
                        ...
                        border-radius: 4px;">delete
                         ...
                         <?php
                          ...
                          ?>
                              <?php
                  $sql="SELECT * FROM dorm_record ";
                  $res=mysqli_query($conn,$sql) or die( mysqli_error($conn));
                  $rows=mysqli_num_rows($res);
                  if($rows>0){
                    while ($rows=mysqli_fetch_assoc($res)){
                      $pr_id=$rows["property_id"];
                      $p_name=$rows["property_name"];
                      $date_given=$rows["date_given"];
                   ...
                   ?>
                      ...
                      <div class="edit-delete-btn" style="display: flex; flex-direction:column;
                      align-items: center;">
                       <a href="delete_record.php?deleteid=<?php echo $pr_id?>&tn=<?php echo $table_name; ?>&sti=<?php echo $student_id; ?>&hdr=<?php echo $header; ?>">
                       <button style="background-color: red; color:white;
                        border:none;
                        padding:6pt;
                        width: 80px;
    ...
    <div class="popup-profile-container" id="popup-profile">
      ...
      <div class="profile-detail">
        ...
        <p id="firstnm"><?php echo $_SESSION['dormitory_full_name']; ?></p>
        </div>
        <hr>
        <div class="detail">
          <label for="dept">ID :</label>
          <p id="dept"><?php echo $_SESSION['dormitory_id']; ?></p>
        </div>
        <hr>
        <div class="detail">
          <label for="age">Age :</label>
          <p id="age"><?php echo $_SESSION['dormitory_ag']; ?></p>
        </div>
        <hr>
        <div class="detail">
          <label for="sex">Sex :</label>
          <p id="sex"><?php echo $_SESSION['dormitory_sx']; ?></p>
        </div>
        <hr>
        <div class="detail">
          <label for="year">Qualification :</label>
          <p id="year"><?php echo $_SESSION['dormitory_qualification']; ?></p>
        </div>
        <hr>
         <div class="detail">
          <label for="academic">Position :</label>
          <p id="academic"><?php echo $_SESSION['dormitory_position']; ?></p>
         </div>
      </div>
      <div class="profile-picture-container">
          <div class="profile-picture-bg">
          <?php
             $sql="SELECT * FROM staff WHERE username='".$_SESSION['dormitory_u_name']."' ";
              $res=mysqli_query($conn,$sql) or die( mysqli_error($conn));
              $rows=mysqli_num_rows($res);
              if($rows>0){
                while ($rows=mysqli_fetch_assoc($res)){
           ...
           <div class="changepw-buttons">
              ...
              <?php
                  if(isset($_POST["change_pw"])){
                    ...
                    $sql_change_password=mysqli_query($conn,"SELECT * FROM login WHERE username='".$_SESSION['dormitory_u_name']."'") or die(mysqli_error($conn));
                    ...
                    while ($rows=mysqli_fetch_assoc($sql_change_password)){
                        ...
                        if($new_password==$confirm_password && $old_password == $old_pw){
                          $sql_change=mysqli_query($conn,"UPDATE login SET password='$new_password' WHERE username='".$_SESSION['dormitory_u_name']."' ") or die(mysqli_error($conn));
