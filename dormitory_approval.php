...
<head>
    ...
    <link rel="stylesheet" href="css/Staff_approval/staff_approvalSmallScreen.css">
...
<body>
    <nav>
        ...
        <div class="texts-container">
            ...
            <p><u>Dormitory</u> Clearance approval process</p>
        </div>
        ...
        <div class="left-side-container">
          <div class="approval-details-container">
            ...
            <?php
              $msg="";
              $status_msg="";
                $sql="SELECT * FROM dorm_record WHERE student_id='$manage_id'";
                $res=mysqli_query($conn,$sql) or die( mysqli_error($conn));
                  $rows=mysqli_num_rows($res);
                  if($rows>0){
                    while ($rows=mysqli_fetch_assoc($res)){
                      $msg=$rows["property_name"];
            ...
            <div class="detail allApprove">
                <label for="id_num">Status:</label>
                <p><?php echo $status_msg; ?></p>
            </div>
            <div class="detail allApprove">
                <label for="id_num">Signature:</label>
                ...
                <?php
                  ...
                  $reg_query=mysqli_query($conn,"SELECT * FROM dormitory_final_approve_reject WHERE student_id='$manage_id' ") or die(mysqli_error($conn));
                  $rows=mysqli_num_rows($reg_query);
                  if($rows>0){
                    echo "<p style=color:red;>Already approved...</p>"; 
                  }else{
                    $date = date('Y-m-d H:i:s');
                  $sql = mysqli_query($conn,"INSERT INTO dormitory_final_approve_reject (student_id,status,signature,staff_name,date) VALUES('$manage_id','$status_msg','$file_name','".$_SESSION['dormitory_full_name']."','$date')") or die(mysqli_error($conn));
                    $sql_delete_request=mysqli_query($conn,"DELETE FROM request_dormitory WHERE id='$manage_id'") or die(mysqli_error($conn));
                    $sql_delete_request=mysqli_query($conn,"DELETE FROM dormitory_rejected_requests WHERE student_id='$manage_id'") or die(mysqli_error($conn));
                    echo "<h4 style=color:green;margin-top:20px;>Clearance approved successfully...</p>";
                    $_SESSION['dormitory_approved']=$manage_id;
                  }
                }
                  ?>
                  ...
                  $status_msg="Rejected";
                  $sql = mysqli_query($conn,"INSERT INTO dormitory_rejected_requests (student_id,status,staff_username) VALUES('$manage_id','$status_msg','".$_SESSION['dormitory_full_name']."')") or die(mysqli_error($conn));
